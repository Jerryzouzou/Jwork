...
public class WebHelpActivity extends ActionBarActivity {
    ...
    private void init() {
        ...
        wv.getSettings().setJavaScriptEnabled(true);//设置js脚本可以运行
        wv.setScrollBarStyle(View.SCROLLBARS_INSIDE_INSET);//设置滚动条样式
        wv.setWebViewClient(new WebViewClient(){// 设置webviewclient（web登录器）
        ...
        wv.setWebChromeClient(new WebChromeClient(){ // 载入进度改变而触发,如果全部载入,隐藏进度对话框
            @Override
